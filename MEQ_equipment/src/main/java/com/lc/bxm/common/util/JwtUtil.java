...
public class JwtUtil {
     ...
     * @param user  用户的对象
     */
    public static Claims parseJWT(String token, String userId) {
        //签名秘钥，和生成的签名的秘钥一模一样
        String key = userId;
        //得到DefaultJwtParser
    ...
    /**
     * 校验token,这里校验的是token中携带的密码于数据库一致的话就校验通过
     */
    public static Boolean isVerify(String token, String userId) {
        //签名秘钥，和生成的签名的秘钥一模一样
        String key = userId;
        //得到DefaultJwtParser
        Claims claims = Jwts.parser()
